...
const Stats = () => (
  <section className="py-24 border-t border-white/10 relative z-10 bg-white/[0.01]">
    <div className="max-w-7xl mx-auto">
      ...
      <div className="mt-16 text-center font-mono text-[10px] uppercase tracking-[0.4em] text-neutral-600">Built on experience, precision, and customer satisfaction</div>
    </div>
  </section>
);
        ...
        <div className="lg:col-span-7">
          <PremiumCard className="h-full !p-0">
            <div className="w-full h-[300px] md:h-[500px] bg-neutral-900 border border-white/5 rounded-lg overflow-hidden relative">
              <iframe width="100%" height="100%" style={{ border: 0, filter: 'grayscale(1) invert(1) hue-rotate(180deg)' }} loading="lazy" allowFullScreen referrerPolicy="no-referrer-when-downgrade" src="https://www.google.com/maps?q=Joshy's+ECM+Diagnostic+Service+Vellanikkara+Thrissur&output=embed"></iframe>
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-black text-white font-sans selection:bg-white selection:text-black relative overflow-x-hidden">
      ...
      <ServicesDirectoryModal isOpen={isAllServicesOpen} onClose={() => setIsAllServicesOpen(false)} onBook={() => { setIsAllServicesOpen(false); setIsBookingOpen(true); }} />

      <div className="fixed inset-0 z-0 overflow-hidden smooth-render bg-black">
        <motion.video initial={{ opacity: 0 }} animate={{ opacity: videoLoaded ? 0.6 : 0 }} transition={{ duration: 2, ease: "easeInOut" }} autoPlay loop muted playsInline onCanPlayThrough={() => setVideoLoaded(true)} className="absolute inset-0 w-full h-full object-cover scale-[1.1] md:scale-105 md:object-center object-[25%_center] pointer-events-none">
